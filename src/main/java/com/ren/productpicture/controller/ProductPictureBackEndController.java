...
public class ProductPictureBackEndController {
    ...
    public ResponseEntity<Map<String, Object>> uploadChunk(
        ...
        // 更新進度
        Integer progress = (chunkNumber * 100) / totalChunks;
...
//    }
    ...
    @GetMapping("DBPhotoReader")
    public void dBGifReader(@RequestParam("productPicNo") Integer productPicNo, HttpServletResponse res)
            throws IOException {
        // 設定圖片格式
        String mimeType = productPictureSvc.getOneProductPicture(productPicNo).getMimeType();
        res.setContentType(mimeType);
        ServletOutputStream out = res.getOutputStream();

        try {
            if (validateFileType(mimeType)) {
                out.write(productPictureSvc.getOneProductPicture(productPicNo).getProductPic());
            } else {
                out.write(productPictureSvc.retrieveFile(productPicNo));
            }
        } catch (Exception e) {
