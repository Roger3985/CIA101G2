...
public class RentalPicControllerBackEnd {
    ...
    //顯示後台 updateRentalPic
    @GetMapping("/updateRentalPic")
    public String updateRentalPic(ModelMap model) {
        List<RentalPic> rentalPicList =rentalPicService.findAll();
        model.addAttribute("rentalPicList", rentalPicList);
        model.addAttribute("rentalPic", rentalPicList.get(0));
    ...
    public String getOneDisplay(@RequestParam(value = "rentalPicNo",required = false) Integer rentalPicNo, ModelMap model) {
        ...
        model.addAttribute("rentalPic", rentalPic);
        return "/backend/rentalpic/listOneRentalPic"; // 查詢完成後轉交
    }
    ...
    public String updateRentalPic(@Valid RentalPic rentalPic, BindingResult result, ModelMap model,
        ...
        // 去除BindingResult中rentalFile欄位的FieldError紀錄 --> 見第172行
        result = removeFieldError(rentalPic, result, "rentalFile");

        if (parts[0].isEmpty()) { // 使用者未選擇要上傳的新圖片時
            byte[] rentalFile = rentalPicService.findByRentalPicNo(rentalPic.getRentalPicNo()).getRentalFile();
            rentalPic.setRentalFile(rentalFile);
        } else {
            for (MultipartFile multipartFile : parts) {
                byte[] rentalFile = multipartFile.getBytes();
                rentalPic.setRentalFile(rentalFile);
            }
        }
        if (result.hasErrors()) {
        ...
        rentalPic = rentalPicService.findByRentalPicNo(Integer.valueOf(rentalPic.getRentalPicNo()));
        model.addAttribute("rentalPic", rentalPic);
        return "/backend/rentalpic/listOneRentalPic"; // 查詢完成後轉交
    }
    ...
    public String getOneUpdate(@RequestParam("rentalPicNo") String rentalPicNo, ModelMap model) {
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    public String addRentalPic(@Valid RentalPic rentalPic , BindingResult result, ModelMap model,
        ...
        return "redirect:/backend/rentalpic/listAllRentalPic";
    }


    @GetMapping("DBGifReader")
    public void DBGifReader(@RequestParam("rentalPicNo") String rentalPicNo, HttpServletRequest req, HttpServletResponse res)
            throws IOException {
        res.setContentType("image/gif");
        ServletOutputStream out = res.getOutputStream();

        try {
            out.write(rentalPicService.findByRentalPicNo(Integer.valueOf(rentalPicNo)).getRentalFile());

        } catch (Exception e) {  // 如果發生異常，輸出預設的佈景圖片
            byte[] buf = Base64
                    .getDecoder()
...
//     */
//    @PostMapping("/json")
//    public String handleJson(@RequestBody RentalPic rentalPic){
//        System.out.println(rentalPic);
//        return "finish";
//    }
     ...
     */
    @ModelAttribute("rentalPicData") // 以rentalNo搜索，取出對應的rental資料庫
