...
<html>
<head>
    <title>fallElove ProductCategory Manage</title>
...
<p>This is the Home page for FallELove ProductCategory Management</p>
...
<ul>
    <li><a href='listAllProductCategory.jsp'>List</a> all ProductCategory. <br> <br></li>


    <li>
        <FORM METHOD="post" ACTION="productCategory.do">
            <b>輸入商品類別編號 (從1開始):</b> <input type="text" name="pCatNo">
    ...
    <jsp:useBean id="productCategorySvc" scope="page"
                 class="com.ren.productcategory.service.ProductCategoryServiceImpl"/>

    <li>
        <FORM METHOD="post" ACTION="productCategory.do">
            <b>選擇商品類別編號:</b> <select size="1" name="pCatNo">
            <c:forEach var="productCategoryVO" items="${productCategorySvc.all}">
                <option value="${productCategoryVO.pCatNo}">${productCategoryVO.pCatNo}
                </option>
            </c:forEach>
    ...
    <li>
        <FORM METHOD="post" ACTION="productCategory.do">
            <b>選擇商品名稱:</b> <select size="1" name="pCatName">
            <c:forEach var="productCategoryVO" items="${productCategorySvc.all}">
            <option value="${productCategoryVO.pCatNo}">${productCategoryVO.pCatName}</option>
            </c:forEach>
        </select> <input type="hidden" name="action" value="getOne_For_Display">
            <input type="submit" value="送出">
        </FORM>
